...
class getID3_cached_mysql extends getID3
{
	...
	function getID3_cached_mysql($host, $database, $username, $password) {
		...
		$version = '';
		if ($this->cursor = mysql_query("SELECT `value` FROM `getid3_cache` WHERE (`filename` = '".mysql_real_escape_string(getID3::VERSION)."') AND (`filesize` = '-1') AND (`filetime` = '-1') AND (`analyzetime` = '-1')", $this->connection)) {
